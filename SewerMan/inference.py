...
def evaluate(dataloader, model, device):
    ...
    with torch.no_grad():
        for i, (images, imgPaths) in enumerate(dataloader):
            ...
            #sigmoidOutput = output.detach().cpu().numpy()
...
def run_inference(args):
    ...
    model.load_state_dict(updated_state_dict,False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=300, help="Size of the batch per GPU")
    parser.add_argument('--workers', type=int, default=8)
    parser.add_argument("--model_path", type=str, default='./logs/zhong2023_multilabel/e2e-version_1/.ckpt')

    parser.add_argument("--best_weights", action="store_true",  default=True,help="If true 'model_path' leads to a specific weight file. If False it leads to the output folder of lightning_trainer where the last.ckpt file is used to read the best model weights.")
