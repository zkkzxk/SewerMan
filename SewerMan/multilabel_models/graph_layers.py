...
class ConvGraphCombination(nn.Module):
    ...
    def forward(self, cnn_feature, gcn_feature):
        ...
        cnn_feature = torch.einsum('bhwc,blc->bhwl',cnn_feature.to('cuda'),gcn_feature.to('cuda'))
        #cnn_feature = self.r(cnn_feature)
        #print("Einsum输出范围:", cnn_feature.min(), cnn_feature.max())
        # mean = torch.mean(cnn_feature)  # 输出: tensor(4.5000)
        # variance = torch.var(cnn_feature)  # 输出: tensor(5.2500)
        # print(mean)
        # print(variance)
        ...
        #print("主分支输出范数:", torch.norm(cnn_feature))
        #print("残差项范数:", torch.norm(cnn_input * 0.1))
        return cnn_feature
